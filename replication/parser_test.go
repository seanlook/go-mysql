...
func TestFlashbackTimestampUpdate(t *testing.T) {
	// 使用与 TestRowsEventDecodeFunc 相同的测试数据
	// FORMAT_DESCRIPTION_EVENT
	...
	// 原始事件的时间戳（从 header 前 4 字节读取）
	originalTimestamp := binary.LittleEndian.Uint32(rowsData[0:4])
	...
	// 验证至少有事件产生
	require.True(t, len(events) > 0, "应该产生至少一个事件")

	for _, ev := range events {
		if ev.Header.EventType == FORMAT_DESCRIPTION_EVENT {
			continue // FDE 不参与闪回时间戳修改
		}
		...
		require.True(t, ev.Header.Timestamp >= beforeTime && ev.Header.Timestamp <= afterTime,
			"事件时间戳应该在闪回执行时间范围内, got %d, want [%d, %d]",
			ev.Header.Timestamp, beforeTime, afterTime)

		// 验证时间戳不再是原始时间
		require.NotEqual(t, originalTimestamp, ev.Header.Timestamp,
			"事件时间戳不应该是原始时间")

		// 验证 rawData 中的时间戳与 Header 一致
		if len(ev.RawData) >= 4 {
			rawTimestamp := binary.LittleEndian.Uint32(ev.RawData[TimestampPos : TimestampPos+4])
			require.Equal(t, ev.Header.Timestamp, rawTimestamp,
				"rawData 中的时间戳应该与 Header.Timestamp 一致")
		}

		// 验证 CRC32 checksum 正确性（如果事件有 checksum）
		if len(ev.RawData) > EventHeaderSize+4 {
			// 最后 4 字节是 checksum
			dataWithoutChecksum := ev.RawData[:len(ev.RawData)-4]
			expectedChecksum := crc32.ChecksumIEEE(dataWithoutChecksum)
			actualChecksum := binary.LittleEndian.Uint32(ev.RawData[len(ev.RawData)-4:])
			require.Equal(t, expectedChecksum, actualChecksum,
				"CRC32 checksum 应该正确, 事件类型: %s", ev.Header.EventType)
		}
	}
}
